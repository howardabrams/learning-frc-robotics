...
public class Robot extends SimpleRobot {
	...
	/**
	 * Holds the Robot's current 'state'. Calling the {@link #go()} function
	 ...
	 */
	public Robot(String name, int x, int y, int direction) {
		this.name = name;
		this.x = x;
		this.y = y;
		this.holding = null;
	...
	/**
	 * A Robot represents a general robot somewhere on a particular playing
	 * {@link Field}.
	 *
	 * @param name
	 *            identify label
	 ...
	 *            'y' coordinate
	 * @param direction
	 *            angle in degrees, where 0° is towards the right
	 * @param field
	 *            playing field where its sensors can now 'see' the balls
	 */
	public Robot(String name, int x, int y, int direction, Field field) {
		this(name, x, y, direction); // calling the other constructor
		this.field = field;
		this.eyes = new Sensor(field, this);
	}
	 ...
	 * loop' from the robot's "owner", e.g. the {@link Field}.
	 */
	public void go() {
		switch (state) {
		case HUNT:
	...
	public String toString() {
		return String.format("Robot %s at (%d,%d) direction=%d", name, x, y, direction);
	}
